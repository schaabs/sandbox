...
namespace cypher
{
    ...
    public class PasswordEncryptionProvider
    {
        ...
        public PasswordEncryptionProvider(string password)
        {
            ...
            ContentCryptoProvder.Padding = PaddingMode.PKCS7;
        }
        ...
        public async Task EncryptFileAsync(FileStream encrypted, FileStream decrypted, CancellationToken cancel)
        {
            ...
            while ((bytesRead = await decrypted.ReadAsync(inblockbuff, 0, inblockbuff.Length)) == inblockbuff.Length)
            {
                encryptor.TransformBlock(inblockbuff, 0, inblockbuff.Length, outblockbuff, 0);

                for (int i = 0; i < outblockbuff.Length; i++)
                {
                    encrypted.WriteByte(filebuff[bytesWritten % filebuff.Length]);
            ...
            encrypted.Write(outblockbuff, 0, outblockbuff.Length);

            bytesWritten += outblockbuff.Length;

            encrypted.SetLength(bytesWritten);

            encrypted.Flush();
        }

        public async Task DecryptFileAsync(FileStream encrypted, FileStream decrypted, CancellationToken cancel)
        {
            await DecryptHeaderAsync(encrypted, cancel);
            ...
            while ((bytesRead = await encrypted.ReadAsync(inblockbuff, 0, inblockbuff.Length)) == inblockbuff.Length)
            {
                encryptor.TransformBlock(inblockbuff, 0, inblockbuff.Length, outblockbuff, 0);
                
                await decrypted.WriteAsync(outblockbuff, 0, outblockbuff.Length);
            ...
            decrypted.SetLength(contentBytes);
        }
